...
### Agent Runbook: kash-media Specifics
...
3. **Same-day first-party pins** (a kash-docs released today is inside the 14-day
   ...
   ```toml
   [tool.uv]
   exclude-newer-package = { kash-docs = "<tomorrow>T00:00:00Z" }
   ```
...
4. **If `gh` is unavailable**: annotated tag + push, then trigger `publish.yml` on
   ...
   `curl -s https://pypi.org/pypi/kash-media/json` and smoke-test
   `uvx --from kash-media==X.Y.Z kash --version`.
